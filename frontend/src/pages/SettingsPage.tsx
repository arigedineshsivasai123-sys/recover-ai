...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-extrabold text-white tracking-tight">Recovery Safety Rules & Merchant Controls</h1>
        <p className="text-sm text-slate-400">Configure deterministic financial safety rules enforced server-side</p>
      </div>

      <div className="glass-panel border border-slate-800 rounded-3xl p-6 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <div className="font-bold text-white text-base">Automatic Revenue Recovery</div>
            <div className="text-xs text-slate-400">Allow AI agent to execute automatic recovery flows within safety bounds</div>
          </div>
          <button
            ...
            className={`w-12 h-6 rounded-full transition p-1 flex items-center ${
              settings.auto_recovery_enabled ? 'bg-emerald-500 justify-end' : 'bg-slate-800 justify-start'
            }`}
          >
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[11px] text-slate-400">Transactions above this amount strictly require manual merchant approval.</p>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500"
            />
            <p className="text-[11px] text-slate-400">Prevents customer fatigue and duplicate payment attempts.</p>
          </div>
        ...
        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2 text-xs">
          <div className="font-bold text-slate-200 flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> Active Server Safety Policy
          </div>
        ...
        <div className="pt-2 flex items-center gap-3">
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition"
          >
